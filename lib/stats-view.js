...
export default class StatsView extends EventEmitter {
   ...
   render ({session, today, history}) {
      this.element.innerHTML = `
         <div class="timer-stats">
            <h1 id='timer-title'><span class='icon icon-hourglass'></span></h1>
   ...
   */
   currentSession ({linesAdded, linesDeleted}) {
      return `
         ...
         <p>
            <span class='timer-number'>${linesAdded}</span> lines added &
            <span class='timer-number'>${linesDeleted}</span> lines deleted.
         </p>
      `
   }
   ...
   */
   today ({completions, linesAdded, linesDeleted}) {
      return `
         <h2>Today</h2>
         <p><span class='timer-number'>${completions}</span> sessions completed.</p>
         <p>
            <span class='timer-number'>${linesAdded}</span> lines added &
            <span class='timer-number'>${linesDeleted}</span> lines deleted.
         </p>
