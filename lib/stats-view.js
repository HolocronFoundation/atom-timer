...
export default class StatsView extends EventEmitter {
  ...
  render ({session, today, history}) {
    this.element.innerHTML = `
      <div class="Tiny-Timer-stats">
        <h1 id='Tiny-Timer-title'><span class='icon icon-hourglass'></span></h1>
  ...
  currentSession ({linesAdded, linesDeleted}) {
    return `
      ...
      <p>
        <span class='Tiny-Timer-number'>${linesAdded}</span> lines added &
        <span class='Tiny-Timer-number'>${linesDeleted}</span> lines deleted.
      </p>
    `
  }
  ...
  today ({completions, linesAdded, linesDeleted}) {
    return `
      <h2>Today</h2>
      <p><span class='Tiny-Timer-number'>${completions}</span> sessions completed.</p>
      <p>
        <span class='Tiny-Timer-number'>${linesAdded}</span> lines added &
        <span class='Tiny-Timer-number'>${linesDeleted}</span> lines deleted.
      </p>
