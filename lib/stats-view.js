...
export default class StatsView extends EventEmitter {
  ...
  render ({session, today, history}) {
    this.element.innerHTML = `
      <div class="tinytimer-stats">
        <h1 id='tinytimer-title'><span class='icon icon-hourglass'></span></h1>
  ...
  currentSession ({linesAdded, linesDeleted}) {
    return `
      ...
      <p>
        <span class='tinytimer-number'>${linesAdded}</span> lines added &
        <span class='tinytimer-number'>${linesDeleted}</span> lines deleted.
      </p>
    `
  }
  ...
  today ({completions, linesAdded, linesDeleted}) {
    return `
      <h2>Today</h2>
      <p><span class='tinytimer-number'>${completions}</span> sessions completed.</p>
      <p>
        <span class='tinytimer-number'>${linesAdded}</span> lines added &
        <span class='tinytimer-number'>${linesDeleted}</span> lines deleted.
      </p>
