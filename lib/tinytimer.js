...
export default {
   ...
   activate () {
      ...
      this.subscriptions.add(atom.commands.add('atom-workspace', {
         'tinytimer:start': () => this.start(),
         'tinytimer:stop': () => this.stop(),
         'tinytimer:reset': () => this.reset(),
         'tinytimer:pause-or-resume': () => this.pauseOrResume()
      }))
   ...
   */
   loadState () {
      const stateStr = window.localStorage.getItem('tinytimer') || {}
      try {
         const state = JSON.parse(stateStr)
         this.statsTracker = new StatsTracker(state.stats)
      } catch (err) {
         console.log(err)
         window.localStorage.removeItem('tinytimer')
   ...
   */
   serialize () {
      ...
      window.localStorage.setItem('tinytimer', JSON.stringify(toSave))
   },
   ...
   pauseOrResume () {
      ...
      if (this.paused) {
         ...
         this.editorObserver.once('editor_did_change', () => {
            if (this.paused && atom.config.get('tinytimer.typeToResume')) this.pauseOrResume()
         })
      }
